...
// The need for IDEs for dep typed langs and itps
This is because dependent types can be much more precise about the programs they describe. The type can encode invariants that must hold during the function's execution, or preconditions on the parameters. Compared to non-dependently typed languages, writing programs in dependently typed languages without tools for querying the types of terms or holes is difficult. Therefore, many dependently typed langauges need interactive development environments (IDEs) to be useful to programmers.

Additionally, dependently typed languages are also often used as theorem provers. However without tooling, such a system can be hard to prove theorems in, especially since normalisation of the system can be hard to reason about without querying the system. Additionally, these systems benefit even more from an IDE -- which for theorem provers is often called an _interactive theorem prover_ (ITP) -- since an ITP can also provide helpful tooling to construct new proofs.

#figure(caption: [Example of using Lean4, an interactive theorem prover, to prove a theorem. The panel on the right shows information about the current state of the proof and what remains to be done based on the location of the cursor.], image("itp.png")) <itp>

Juhošová et al. @learning-obstacles have investigated what obstacles students experience when learning Agda as an interactive theorem prover. Their advice is to improve robustness and accessibility, where one point they mention for both improvement areas is polishing and improving the existing tooling.

Juhošová et al. @juhovsova2026way also conducted a study about type driven development. The participants of the study were asked to give improvements to languages that offer type-driven development and their tooling. Some of the improvements the participants suggested were "wider editor support for interactive modes", "better IDE support for large-scale proof engineering" and more refactorings. Backx @dtitp-study has also investigated how novice dependently-typed language programmers write code, but also includes experienced programmers in his study. He, too, saw that users experience a lack of tooling support for refactorings, and also recommends authors of dependently-typed interactive theorem provers to improve the editor tooling with, for example, a search feature for lemmas, or refactoring features that can also be found in non-dependently typed languages.

Focusing on Agda in particular, even though there is strong incentive from users to develop new features, we see little innovation in editor features: all of the extensions for all currently available editors support roughly the same feature set. These editor features are provided by the compiler, and the extensions all merely implement an interface to these compiler features from an editor. One reason for the lack of innovation might be that extending this feature set is quite difficult to do: the features are implemented within the Agda compiler, which is a large Haskell code base. Additionally, these features are quite tightly coupled to Agda's elaborator and unification system, which are complex systems that are hard interact with without much prior experience.
...
Extensions can be written in Javascript or Typescript. When VS Code executes the code for an extension, it provides an API which allows extensions to interface with the different parts of the editor. Some examples of controllable elements from an extension are highlighted in @vscode-fig.
...
- Agda has many different *commands* that users can invoke using keybinds with their cursor at certain locations. These commands can be categorised in roughly three categories.
  #grid(columns: (1fr, 1fr), gutter: 16pt)[
    _Goal refinement commands_ allow the user to enter (parts of) expressions into a goal and Agda will type-check or refine them based on the goal's type. Examples of commands in this category are:
      ...
      - the make case command, which creates exhaustive pattern matching clauses for a parameter, where possible. For example, when pattern matching on a constructor, make case will create a clause matching constructor.
  ][
    #align(
      horizon,
  ...
  _Goal display commands_ give information about the goal under the cursor, which is displayed in the display buffer. This information can help the user see what they need to prove and what proofs are available in the current context. Examples of commands in this category are:
  ...
  _General Commands_ are commands that can be executed on a module or an arbitrary expression. Examples include
    - the load command, which type checks the current module and returns syntax highlighting;
...
#grid(columns: (50%, 50%), gutter: 16pt)[
  - The *display buffer* is a panel accompanying the editing buffers, where information about the state of the extension is displayed. For example, when the load file command is invoked the compiler shares information about its progress type-checking the current file and its dependencies. After type-checking completed, the buffer can show either information about goals -- if the type-checking was successful -- or it can show information about type errors in the current file or its dependencies. 
][
  #align(horizon)[
...
#grid(columns: (60%, 40%), gutter: 16pt)[
  - *Syntax highlighting*: Once the current file is successfully type-checked, the compiler also shares highlighting information to the extension. Agda provides _semantic highlighting tokens_, which means that the tokens are coloured based on what they represent. For example, instead of colouring a variable black regardless of its value -- Agda will colour a variable referencing a function blue. Additionally, Agda provides syntax highlighting for different error types, and the locations of where they occur.
][
...
Metaprogramming refers to the process of writing programs that modify or generate other programs. In this thesis we mainly focus on static metaprogramming, that is metaprograms which evaluate at compile-time to syntax trees, which are used for compilation or further elaboration. @metaprogramming-example shows a contrived example metaprogram in Agda. The metaprogram on the left builds up the syntax tree for an identity function, which can be spliced in using `unquote` as seen on the right.

#figure(caption: [Example of a metaprogram in Agda. The function on the left defines how to build up the term for the identity function, the function on the right uses the metaprogram to define the identity function.], grid(columns: (1fr, 1fr), gutter: 16pt)[
  ```agda
  mk-Id : TC Term
  mk-Id = returnTC
    (lam hidden
...
Idris1 has static metaprogramming @christiansen2016elaborator. Terms can be quoted, which converts the term to a Idris1 data type representing Idris1 terms, called `TT` (@tt-def). This data type also has a constructor for binders, which can create let bindings to introduce new variables. There is also an inverse of quotation, called unquotation. The term `%runElab m`, where `m : Elab TT` evaluates the metaprogram and the returned term of type `TT` is converted back into a unquoted Idris1 expression, which is later elaborated or compiled.

Metaprograms in Idris1 are written using a tactic-based embedded domain specific language (EDSL). The tactics are functions that update the "proof" state, which comprises among other technicalities, the hole queue and the incrementally built proof term. The tactics are defined from within the compiler and have the ability to invoke internal function in the elaborator. For example, the `fill` tactic used in @idris1-meta-example places another quoted expression in the first hole of the hole queue and unifies its type with the expected type of the hole. If this fails, the compiler presents the user with a type error.
][
#stack(spacing: 32pt)[
...
#figure(caption: [An example taken from @christiansen2016elaborator. It demonstrates how to build up the expression `Z + S Z` using the metaprogramming monad.])[
```Idris1
do
...
Similar to Idris1, Agda also has static metaprogramming. Agda has data types for terms, clauses and definitions, among others. The `Term` data type reflects the structure of elaborated terms in Agda. This means, for example, that let and where bindings are already substituted and cannot be inspected or generated using metaprogramming. This is generally not a problem, though. The output of metaprograms in Agda is not inspected (unless debugging metaprograms), but immediately spliced into the syntax tree and used for further elaboration.

Another difference with Idris1 is that Agda does not use a tactic-based EDSL. Metaprograms are written in the `TC` monad, whose definition is opaque. The reflection API that Agda exposes defines a number of primitive functions that all operate in the `TC` monad. This monad holds the environment, and the metavariable state, among other technicalities.

=== In Lean4 <lean4-meta>

Lean4 is another dependently-typed language and interactive theorem prover @lean4. The language allows metaprogamming in different ways in a Lean program:
- In Lean4, all top-level statements, including `def`, `inductive` are called commands. Besides these builtin commands, the user has the ability to define their own commands. The definition of a command includes a metaprogram of type `Syntax → CommandElabM Unit`. The function receives the abstract syntax tree of the argument to the command, and must produce some side effects, which include updating or querying the environment or performing `IO` operations. @lean-command illustrates these components of a command.
  #figure(caption: [An example command in Lean4, and how Lean4 calls the metaprogram associated with the command.])[
  ```lean
  #check 2 + 2
...
- Macros can access the environment and the local metavariables.
- Tactics can access the environment, local metavariables and the proof state. The functionality of a tactic is defined using the `TacticM` monad, which expands directly upon the `TermElabM` monad.

A metaprogram in Lean4 looks similar to one in Agda, where you can call functions in a monadic non-tactic style. Whereas Agda only allows functions from the reflection API to be called, the Lean4 compiler is written in Lean itself, and allows metaprograms to directly call any function defined in the compiler. This means that metaprograms in Lean4 can use any part of Lean4's elaborator.

Lean4 also has extensible user interface inspired by Korkut et al. @extensible-editor, which allows user to execute metaprograms at edit-time and display their results in visualisation in the infoview @proofwidgets. @proofwidget-demo shows an alternative implementation for the `#check` command, where the user can type an expression in the infoview (on the right) and the type of the expression is inferred and displayed. When the user puts their cursor on a `#widget` command in the source file, the `lean4-vscode` extension loads the widget's React code into the infoview, from where the widget can call metaprograms (of type `TermElabM`) using the LSP to the compiler.
...
One result of this thesis is a working implementation of a Visual Studio Code extension written in Agda -- which compiles to Javascript -- that supports the same set of features other Agda extensions also have.

The extension acts as the mediator between Agda and Visual Studio Code. Agda exposes an language-agnostic API with which the extension can communicate. The extension can communicate with Visual Studio Code through the use of the Javascript API. @high-level-arch shows the high-level architecture of the extension and how the different parties communicate with each other.
...
+ The user makes an edit in the buffer in Visual Studio Code or triggers a keybind.
+ Visual Studio Code listens for keybinds configured by the extension, and invokes the handler that is registered in the extension.
+ In the handler, the extension defines what to send to the Agda process -- which typically includes which file the interactions works on, and how Agda should respond -- and awaits the response.
+ Agda responds with many different responses to the sent interaction, and notifies the extension that new interactions may be sent at the end. The responses are encoded in JSON, and have a kind, which can be used to distinguish what the response's content is about. Responses can contain new highlighting tokens, information about the progress of type-checking, or formatted errors and warnings.
+ Each response is decoded into Javascript objects and then parsed according to the kind property. Each of the parsed responses is then handled individually. Handling a repsonse typically involves updating the internal caches with potentially new locations for tokens and interactions points.
+ After the caches are updated, the extension sends change requests for the buffer or updating the display buffer via the Visual Code Studio API.

The sections below each elabortate in more detail about each of the steps described above. In particular, @activation discusses the activation of the extension and the registration of the handlers. @sending-interactions discusses how interactions are encoded and sent to the Agda process. @receiving-handling discusses how responses are parsed and handled. @change-responses discusses how edits in the buffer are handled, and why it is necessary to handle changes.

== Anatomy of a Visual Studio Code extension <activation>
...
Visual Studio Code also provides ways to run your extension both in debug mode as in production mode. The debug mode can be triggered from VS Code's debug menu and runs a special `extensionHost` debug profile, which opens a new window with the extension loaded into it. Any output on the standard output is redirected to the main window's debug output, and additionally breakpoints can be set. To publish an extension, VS Code has a tool called `vsce`, which can package all the files needed to run the extension into a compressed binary VSIX file. This file is then published to the marketplaces.

One issue is that `vsce` only packages the files you tell it to package. Additionally, even though the `vsce` utitily already compresses the extension files, but Visual Studio Code recommends to keep the source code for the extension itself as small as possible still. Following these guidelines, we cannot use put the `node_modules` folder -- the folder containing all dependencies, both runtime and buildtime -- in the extension package. For Avea, this folder can easily grow as large as a hundred megabytes, most of which is dedicated to build time tools. This would be a waste to include in the extension package, and therefore packaging requires some preprocessing step on the source code. For this, Avea uses another tool called `esbuild` -- a bundler for Javascript -- to combine the generated Javascript source code by Agda with the source code of just the runtime dependencies in `node_modules`, and to minify the resulting bundle, which is then put in the extension package. @js-infra shows a diagram of how the tools call each other.

Once the extension is loaded -- either in a debug window or using the production extension package -- the code in the entry file is executed. The entry file sets up the dependencies -- discussed in more detail in @libraries -- and then calls the activation function in Avea. This activation function has two purposes: it creates all of the resources that need to be created in an `IO` context, and it registers all of the handlers for keybinds, commands, hover-, semantic token and defintion providers. The `activate` function serves a very similar purpose as the `main` function does in regular Agda programs, where all `IO` operations have to happen in `activate` or a function called from there.

The handlers for the keybinds and commands and such keep listening until they are explicitly deactivated. It is the extension's responsibility to shut down the event listener or kill the process every time a long-running event handler or process is started. When creating handlers for events in the VS Code API, they return a `Disposable`, which is simply an object that has a `dispose` method. `Disposable`s can be registered in the extension's context object, which is the only parameter in the entry file's `activate` function. After registering, VS Code will run all the `dispose` functions registered in the context, on _deactivation_.

#text(red)[TODO: Provide the Agda encoding of Disposables in Avea.]

Deactivation happens just before the extension should no longer be active, for example, when a non-Agda file is opened, or the extension is uninstalled or disabled, depending on the activation events. Besides an `activate` function in the entry file, there is a `deactivate` function, which is called on deactivation, and can be used to define cleanup that cannot be defined using `Disposable`s.
...
+ In the `package.json` file, it should be declared what keybinds or commands should call the handler by name.
+ In the handler, we define what the keybind or command should actually do. Depending on the kind of command, the handler does the following:
...
+ Before the process is spawned, Avea needs to read the settings via the VS Code API, to read the path to the Agda binary. The user can configure multiple paths to different Agda binaries, which can be selected using a keybind within Avea. For each of the binaries, the version is requested, which is used for display and to check whether the binary exists. When the user selects one of the binaries, the Agda path setting is set to that path, which is used when spawning the child process.
+ Sending interactions to Agda is done by spawning a child process running Agda's `--interaction-json` mode, and sending data over the child process's standard input. Avea manages a single Agda process, which is spawned on activation.
...
Lastly, since the child process library is not part of the VS Code API, Avea manually creats a `Disposable` that kills the process gracefully -- removing the attached event listeners as well. #text(red)[TODO: Describe how a Disposable is created ig.]
...
Each handler may cause edits to the buffer or other asynchronous actions. However, if the handlers are invoked immediately from the handler, then there is no guarantee that the actions are executed in a deterministic order. This is because the data handler from Node.JS's child process library uses a callback function from which the promises of the activate function cannot be bound with. So the promises are immediately started and act as separate threads, which can introduce a number of concurrency problems.

To mitigate concurrency problems, Avea uses a concurrent queue which has a limit set to 1 to make sure only one response is handled at a time. This prevents edits from being made in the wrong order, or while other edits are being made, causing VS Code to throw an error.

The output of the data handler is a list of complete Agda responses as a list of parsed JSON objects. These objects are then parsed once again into more specific data types per response type. Each JSON object holds a `kind` property that can be used to distinguish the objects and drive an efficient parser from the JSON objects to Agda data types.
...
#heading(numbering: none, depth: 4)[Handling parsed responses]

After the data handler has collected a number of responses and parsed them to JSON, it remains to parse the JSON responses to specific data types which have more specific information about the response type. These data types can be passed to a handler which calls the relevant VS Code API. @handle-agda-message shows how the handlers are called from the JSON parsers: each decoder function parses their corresponding kind of response and passes a data type to the handler function.

#figure(caption: [JSON decoder for the responses Agda sends])[
```agda
...
Once, the tokens are updated in the internal cache, the handler will send a notification to VS Code for it to retrieve new tokens. VS Code will automatically request new semantic tokens after a small batch of edits are made, but such a tokens request can also be requested by the extension. An example of a situation when this is useful, is when the user loads a file that has not been loaded before. 
...
- Holes are interaction points that are at least four characters wide. Holes contain an expression, which is delimited by goal markers `{!` and `!}`. The expression in the hole need not be a valid Agda expression, both in terms of syntax or in terms of type. Other interactions can operate on the hole, by inspecting the inner expression. These interactions require the latest content of the hole, since Agda does not actively keep track of a hole's contents.
...
#grid(columns: (50%, 46%), gutter: 16pt)[
  This response is specific to the give and refine interactions. It contains a give result -- which is either a new expression, or a command to use the expression currently contained in the hole as the replacement: either the expression in the goal is parenthesised and the goal markers are removed, or the expression is let as-is and the goal markers are removed. See @give-result for the definition in Avea.
][
...
This handler does the following steps:
  + Depending on the give result, the handler queries the VS Code buffer to retrieve the contents of the hole. The give action response contains the id of the interaction point in question, which can be used to search the latest source location in the extension's internal cache.
  ...
  + All expansions are collected into a list of edits that can be made to the VS Code buffer. Additionally, if any expansion have been performed, the cursor is moved to the first new hole, to improve developer experience. 
  + A load interaction is issued, so that the newly created holes will correctly be registered by that interaction's interaction points response. This also means that the internal cache need not be updated since the load interaction will elicit an interation points response.
...
The semantic tokens provider is queried by VS Code whenever the user has edited a buffer. These edits are not communicated to the semantic tokens handler: it needs external listeners to receive updates. The reason these edits are needed in the first place is that Avea saves the locations of all tokens and interaction points. However, when the user edits the buffer these locations might change. Sending the same locations for all tokens will result in pieces of code being highlighted with wrong or incomplete information, for example, identifiers with two colours or missing colours, see @shift-highlighting.

#figure(caption: [Illustration of syntax highlighting in Avea: the left image is a loaded file. The middle images shows an edited buffer when no shifting is performed, whereas the right image shows the same edited file with shifting enabled.], grid(columns: (1fr, 1fr, 1fr), gutter: 16pt, image("pre-highlighting.png"), align(horizon, image("no-shift-highlighting.png")), align(horizon, image("shift-highlighting.png")))) <shift-highlighting>

Initially Avea, stored the tokens as Agda gives them. This means that no processing needs to be done when receiving the tokens, but shifting the tokens requires iterating over the entire list. Typically, a buffer is edited more times than it is loaded, so this approach can cause visual slow downs if the buffer has a large number of tokens. Another approach, which Avea uses now, is to store the tokens in sorted order. This means that the received tokens need to be sorted in $cal(O)(n log(n))$ time. The handler does a binary search to find the first token affected and continue shifting the tokens until a token on a new line. The key observation is that only tokens on the same line after the edit need to be shifted.

== Input mode

There is one component of the extension that operates completely separately from the compiler: the input mode. Technically, the input mode could even run as its own extension, since it shares no resources with the rest of Avea: it does not require access to Avea internal caches. #text(red)[TODO: This feels incomplete, but idk what else to put here yet.]

#figure(caption: [], image("input-mode-state.png", width: 60%)) <input-mode-state>

The input works according to a simple state machine as described in @input-mode-state. Though each transition has some side effects:
+ By default the input mode is disabled. There is one handler that listens for the "\\" character, which enables input mode.
+ Any subsequent characters are used to traverse a trie in a zipper-style. The zipper can be used because a user can type another character to traverse deeper into the trie, but the user can also hit backspace to go up in the trie. A zipper is a perfect structure to perform these traversals on in a purely functional manner. #text(red)[TODO: Maybe add a reference to a paper introducing tries?]
+ If the user types a character that is not a child of the focus node, or the user hits the escape key, then input mode exits and the character is inserted into the buffer.
...
#context {
  ...
  [
    In this chapter, we will look into making Avea extensible. For this we have decided to heavily depend by previous work on editor reflection by Korkut and Christiansen @extensible-editor. They introduced the concept of editor reflection in Idris1 -- a language with a similar metaprogramming library, and a similar compiler structure as Agda.
  ...
  [
    ...
    These drawbacks can be solved by allowing the support code to be written in the same language as the editor actions. Since Avea is written is Agda already, and Agda has similar meta-programming capabilities, Agda is perfect candidate for this. The example from Listing #{n} would be translated to the Agda program in Listing #{n + 1}.
  ]
}
...
- In @edit-actions-agda, we will discuss being able to call meta-programs, which we call *editor actions*, in the Agda compiler. The implementation of this is mostly based on the work of Korkut and Christiansen, but applied on the Agda compiler. Adding editor reflection to the current version of the Agda compiler allows for non-syntactical expression manipulation in holes. For example, simplification of programs written in an EDSL, customised code snippets (e.g. generating boilerplate for type class instances), and proof search actions.

- In @changing-meta-language, we will dicuss why the current reflection API does not include many high-level language constructs (such as `let`, `where`, `do`) or provides source locations. Incorporating these into the reflection API would allow for syntactic changes and more precise edits in the buffer. This enables use cases such as lintings and warnings, which can be executed on every load of a file, and display squiggles in the buffer at exact locations.
...
The starting point for an extension in Avea is a special file that contains all hooks and extension actions. Accompanying this file is a `.agda-lib` file that can be used to declare dependencies upon which the extension actions can build.
...
The extension actions that are executed when a hook triggers, are program written in the `ExtensionAction.t` monad. The `ExtensionAction.t` monad is a `State` monad where the state is Avea's model. However, the monad does not allow any field in the model to be modified: changes or accesses must be done through the defined functions in the Avea library. This way extensions cannot interfere with Avea's internal system, and potentially compromise its other functionality. So examples of functions that operate in this monad are described in @extension-functions.
...
- Implementations of the function described in @extension-functions. These functions already exist within the current implementation, but need to be adjusted to work within the `ExtensionAction.t` monad.
...
#figure(caption: [], image("editor-reflection-complete.png")) <editor-reflection-complete>

*1. Designing the API to call editor actions*

#grid(columns: (6fr, 4fr), gutter: 16pt)[#align(horizon)[
  Recall from @basic-avea-edsl that extensions in Avea are written using a state monad containing Avea's model. This model includes the process handle and response queue needed to communicate to Agda. It is therefore possible to extend the EDSL with functions to call editor actions and handle their results.

  @editor-reflection-1 shows how an extension can interact with Avea and call editor actions.
]][
...
Editor actions are executed in the Agda compiler. However, they are triggered by the extension and receive arguments from there, and the result of the editor action also needs to be communicated back to the extension. To facilitate this commnunication, we can define two type classes: `Encodable` and `Editorable`, whose definitions can be found in @editorable-def. The extension sends values to and from the compiler using JSON. The `Encodable` type class captures the (de)serialisability to and from a type to JSON, and `Editorable A B` combines two `Encodable` instances to capture converibility of values of type `A` to `B` via JSON.
][
  #figure(caption: [], image("editor-reflection-2.png"))
]
...
While the proposed design for editor reflection in Agda can be used by all other Agda language extensions, given that it extends the interaction mode used by all of them. Avea can provide a comprehensible and type-safe API to call editor actions without having to interact with a separate language. This is an edge that Avea holds over all other language extensions.

Taking a broader view to other languages, we see that development in Agda's tooling has been slow over the past few years (#todo[Justify this]) compared to other theorem provers such as Lean4 and Rocq. While there are numerous reasons for the comparatively slow development -- such as development team size, number of users, etc. -- we believe there is also a technical reason. New features in Agda's tooling are highly coupled to the compiler, which is a complex and delicate system to work with. Decoupling the development of tooling from the compiler using editor reflection could boost new development of Agda tooling.
...
+ *Is it possible to make an extensible Agda extension by writing it in Agda itself?* \
  Even though extensibility through editor reflection is also possible for Agda language extensions written in languages other than Agda. Using Agda's meta-programming capabilities provides a better developer experience while keeping the extensibility API type-safe. Tooling developers need not learn Emacs Lisp (or language of choice for the extension) to interface with the editor, and Haskell and the Agda compiler's code base to write new tooling. Instead they can write Agda code to extend Avea, and use the reflection API to interface with the compiler. This extensibility API still allows for many different use cases, and has sufficient expressibility for advanced tools to be created.
...
Writing Avea in Agda has proven to be useful in terms of allowing users to extend the editor tooling in Agda itself. However, with Agda's expertise as a theorem prover, it is also possible to verify parts of Avea's functionality.

*Making the extension editor-agnostic*

#todo[Mention that Lean4 tooling is mainly focussed on VS Code as extra motivation]
...
There are various ways to implement such a reusable "core". One way is to compile the core to WebAssembly, and let the extension for a new editor call functions from the WebAssembly module. Another possibility is to take inspiration from Rocq and Lean4, and to run Avea as a language server, with which a new editor extension can communicate with.
